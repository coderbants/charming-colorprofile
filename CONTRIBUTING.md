...
- To release: `git tag v0.4.3 && git push origin v0.4.3`. The
...
- `v0.4.3.0` — first port release of upstream `v0.4.3`
- `v0.4.3.1` — a hotfix iteration for `v0.4.3` (bug fix released without an upstream
  version bump)
...
release carry the full four-part version (`v0.4.3.1`). `Cargo.toml` keeps the upstream
`X.Y.Z` (`0.4.3`), since crates.io only accepts `MAJOR.MINOR.PATCH`; iteration hotfixes
...
- Commit messages should describe the upstream behaviour being ported or fixed.
...
- Note the pinned upstream tag (`v0.4.3`) in the report.
